...
export class JavascriptCellElement extends HTMLElement {
...
<section class="cell javascript-cell">
  <!-- Cell Types -->
  <cell-type class="cell-types" data-selected="javascript"></cell-type>
...
<hr>`;
  ...
  constructor(){
    ...
    this.shadowRoot!.innerHTML = JavascriptCellElement.template;
  ...
  static fromJSON(incoming: any): JavascriptCellElement {
  ...
  fromJSON(incoming: any): void {
    ...
    for(let output of incoming.outputs){
      if(output.name && output.name === 'stdout') this.log(output.text);
      if(output.name && output.name === 'stderr') this.error(output.text);
  ...
  static fromString(str: string): JavascriptCellElement {
    const obj = JSON.parse(str);
    const cell = JavascriptCellElement.fromJSON(obj);
    return cell;
  }
  ...
  private createIndicator(): any {
    let indicator = this.qs('.cell-output.indicator') as HTMLOutputElement;
    if(!indicator){
